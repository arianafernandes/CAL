...
void newDelivery(Company &comp, User& user) {
	...
	for (unsigned int i = 0; i < comp.getSupermarket().getTrucks().size();
			i++) {
	...
	while (option != 3) {
		do {
			...
			cout << "1 - Modificar o Nome da conta" << endl
					<< "2 - Modificar a Morada" << endl << "3 - Sair" << endl;
...
void areaCliente(Company& comp, User& user) {
	...
	while (option != 4) {
		do {
			cout << "Bem-vindo a tab dos Clientes, " << user.getNif() << "!"
					<< endl;

			cout << "1 - Modificar Conta" << endl << "2- Eliminar Conta" << endl
					<< "3 - Ver perfil" << endl << "4- Sair" << endl;
...
void Login(Company comp) {
	...
	if (user.getNif() == 0) {
		cout << " Nao se encontra registado! Por favor, efetue o registo!"
				<< endl;
	} else {
...
void Clientes(Company& comp) {
	...
	while (option != 3) {
		...
		cout << "1 - Registar-se" << endl << "2 - Login" << endl << "3 - Sair."
				<< endl;
...
void adminSuper(Company& comp) {
	...
	while (option != 4) {
		do {
			...
			cout << "Bem-vindo ao tab de administracao do "
					<< comp.getSupermarket().getName() << endl;
			cout << "1 - Mudar o Nome do Supermercado " << endl
					<< "2 - Modificar o local do Supermercado" << endl
					<< "3 - Modificar Camioes" << endl << "4 - Sair." << endl;
			cout << "Insira o numero da sua escolha. Obrigado." << endl;
			getline(cin, ss);
		} while ((ss.size() == 0));
		option = stoi(ss);
...
void saveUsers(Company comp) {
	...
	for (unsigned int i = 0; i < vecUsers.size(); i++) {
		users << vecUsers[i].getName() << ";" << vecUsers[i].getNif() << ";"
				<< vecUsers[i].getAddressId() << ";" << endl;
	}
	users.close();
}
...
void saveDeliveries(Company comp) {
	...
	for (unsigned int i = 0; i < trucks.size(); i++) {
		...
		for (unsigned int j = 0; j < orders.size(); j++) {
			file << orders[j].getId() << ";" << orders[j].getWeight() << ";"
					<< orders[j].getDate() << endl;
		}
	}
	file.close();
}
